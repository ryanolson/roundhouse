...
**Learner design checkpoint, 2026-09-22.** `DRAFT-online-routing-learner.md` revision 3 proposes strategy arms, sequence inputs, interval credit, durable updates, and offline evaluation. The full proposal is not accepted. Its section 21 accepts only L3b: source-side marks and discovery in the session store. Implementation and backend validation are in progress.

Review models exposed duplicate credit, incorrect cache-write pricing, lost recovery registrations, and two invalid offline estimates. Revised source-discovery rules passed six independent model checks. These checks do not validate production code. The offline proposal now withholds policy-value claims until its evaluation protocol is complete.

Cold-start behavior and exploration are with the owner. Credit attribution, latency basis, local capacity pricing, and deployment thresholds also remain unresolved. No learner policy is enabled by the source-discovery slice. The remaining rows of the completion table still apply.
...
**Repair acknowledgement guard, 2026-09-22.** Commit `a9e7770` strengthens the append-refusal integration test. The test waits for a completed repair before recording the ledger call count. Inspection handles are released before subsequent turns. Premature acknowledgement removal then fails with two ledger calls instead of one. After production restoration, the worker and parent each passed all 23 recovery tests. Formatting passed. The bounded observation window retains scheduling sensitivity. This check covers acknowledgement eviction after append refusal; it does not establish every repair lifecycle invariant. Logs: `/tmp/roundhouse-repair-ack-retention.log` and `/tmp/roundhouse-repair-ack-retention-parent.log`.

**Scoped runtime review, 2026-09-22.** A read-only review of `1638783..8269e47` identified two concerns. Acknowledgement folding uses `Vec::retain`, so each acknowledgement scans the pending settlement backlog despite the bounded scheduling batch. This source observation needs an operation-count regression test before a defect ruling and fix. The reviewer also proposed moving the four classifier orchestration methods into an engine child module, preserving call order and writer ownership. That proposal concerns structure, not an observed behavioral defect. Neither finding establishes whole-PR readiness. Frontier interval feedback and learned routing remain unfinished.

**Acknowledgement scan fix, 2026-09-22.** Commit `f62c621` fixes the confirmed scan. Four acknowledgements visited 58 settlements with a backlog of 16 and 4090 with a backlog of 1024. Missing and repeated acknowledgements also failed the new traversal assertions. Indexed removal now visits four settlement values for either batch, with no full-backlog collection. A call-ID index locates entries in an arrival-ordered map. Replay and interior-removal tests preserve oldest-first scheduling. A counted iterator also confirms that scheduling pulls only three candidates from 2000 pending entries.

The counter excludes hash lookup work and ordered-map comparisons. Its placement is part of the instrumentation; it does not independently detect every possible traversal elsewhere. The result establishes removal of the measured scan, not constant-time removal or a measured latency improvement. The parent passed 63 core tests, 34 runtime tests, and 23 recovery tests independently. After correcting one test-only Clippy finding, the three collection tests, strict workspace Clippy, and formatting passed. The worker also passed seven serialization tests and the workspace compile check. Logs: `/tmp/roundhouse-repair-backlog.log`, `/tmp/roundhouse-repair-backlog-parent.log`, and `/tmp/roundhouse-repair-backlog-parent-final.log`. Independent mutation verification, the engine structure finding, and publication remain pending.

**Independent backlog checks, 2026-09-22.** Three mutations against `f62c621` failed their intended assertions. An instrumented full scan restored the 58-versus-4090 difference. Reversed iteration failed collection and live-session order checks. The session assertion stopped before replay; restored replay passed, but no separate replay mutation failure is claimed. Eager collection retained three candidates while making 2001 iterator calls instead of three, including the final call returning no entry. All sources were restored exactly. The restored suites passed 46 core session tests and 34 classifier runtime tests. The parent inspected the mutation diffs, assertion output, and clean worktree after the worker closed. No edits remained. These checks retain the instrumentation limits stated above. Logs and diffs: `/tmp/roundhouse-repair-backlog-refute.log` and `/tmp/roundhouse-repair-backlog-refute-M{1,2,3}.diff`.

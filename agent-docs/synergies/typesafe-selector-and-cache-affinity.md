...
## Addendum (2026-09-21): cache prediction supplies efficiency evidence

The owner identifies lower-than-expected cache reuse as another feedback signal, especially for destinations without retention guarantees. A shortfall can indicate cache pressure, but the observation alone cannot establish its cause. Prefix changes, cache keys, elapsed time, and provider routing also affect reuse. Quality labels, cache predictions, cost, and latency remain separate inputs. A cache miss must not receive an extra cost penalty that duplicates its measured cost or latency.

Commits `330cacb` and `a4766a1` add the observation layer. The metrics pair each final routed prediction with independently reported terminal usage. They retain separate router and provider token bases. Explicit zero supplies an observation. Missing reports, historical counts without provenance, and locally derived counts do not. The final workspace run passed 1784 tests, with 0 failures and 141 existing ignores. Mutation verification exposed and closed an aggregation defect and a local-path test gap. The observations do not yet update routing. The bandit plan records the evidence and remaining integration.

## Addendum (2026-09-21): judge cache requests

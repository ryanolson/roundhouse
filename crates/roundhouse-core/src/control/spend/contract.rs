...
/// what it must not.
fn monthly_terms(limit_usd: f64) -> BudgetTerms {
    BudgetTerms {
        budget: Budget {
...
///
/// The twin of [`settlement`] and deliberately a separate fixture rather than a
...
pub async fn the_two_settlement_modes_do_not_share_an_identity<L: SpendLedger>(ledger: &L) {
    ...
    assert!(
        ledger
            .settle_grant(settlement(&ada, "turn_1", 1, 1.0, &terms, 0))
            .await
            .unwrap()
            .applied,

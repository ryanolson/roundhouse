...
fn a_turns_first_text_is_measured_from_its_start_on_the_row_that_served_it() {
    ...
    assert_eq!(claude.timing.first_output.samples, 1);
    assert_eq!(claude.timing.first_output.ms_total, 20);
    assert_eq!(claude.timing.first_output.rejected, 0);
}
...
fn an_empty_delta_does_not_stop_the_clock_and_a_later_one_does_not_move_it() {
    ...
    let claude = row(&fold, &claude());
    assert_eq!(
        claude.timing.first_output.samples, 1,
        "one turn, one sample"
    );
    assert_eq!(
        claude.timing.first_output.ms_total, 30,
...
fn a_turn_that_never_speaks_leaves_no_sample_and_no_zero() {
    ...
    assert_eq!(
        (
            claude.timing.first_output.samples,
            claude.timing.first_output.ms_total
        ),
        (0, 0),
...
fn a_delta_with_no_turn_start_in_view_is_not_a_sample() {
    ...
    assert_eq!(
        (
            claude.timing.first_output.samples,
            claude.timing.first_output.rejected
        ),
        (0, 0),
...
fn a_delta_before_its_own_turn_start_is_rejected_rather_than_folded() {
    ...
    let claude = row(&fold, &claude());
    assert_eq!(claude.timing.first_output.rejected, 1);
    assert_eq!(
        (
            claude.timing.first_output.samples,
            claude.timing.first_output.ms_total
...
fn a_superseded_response_contributes_no_latency_sample() {
    ...
    assert_eq!(
        claude.timing.first_output.ms_total, 20,
...
fn one_turn_id_in_two_sessions_supersedes_neither() {
    ...
    assert_eq!(
        both.timing.first_output.samples, 2,
    ...
    for who in [&ada, &bo] {
        ...
        assert_eq!(
            (scoped.timing.first_output.samples, scoped.calls),
            (1, 1),
...
fn a_turn_folded_in_two_batches_measures_what_it_measures_whole() {
    ...
    assert_eq!(
        row(&batched, &claude()).timing.first_output.samples,
        row(&whole, &claude()).timing.first_output.samples,
    );
    assert_eq!(
        row(&batched, &claude()).timing.first_output.ms_total,
        row(&whole, &claude()).timing.first_output.ms_total,
    );
    assert_eq!(
        row(&batched, &claude()).timing.first_output.ms_total,
        20,
...
fn a_project_view_sums_its_members_latency_samples() {
    ...
    assert_eq!(acme.timing.first_output.samples, 2, "ada and bo, never eve");
    assert_eq!(acme.timing.first_output.ms_total, 20 + 30);
    assert_eq!(
        row(&fold, &claude()).timing.first_output.samples,
        3,

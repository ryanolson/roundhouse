...
/// deployment loses the exemption *for the flat spelling* and gets today's
/// behaviour back. Threading the dialect into the signal fold is what would
/// close it, and it would put deployment configuration inside the one part of
/// the validate loop that is a function of the session log alone. The unlock
/// condition, for whoever wants it: a `SignalContext` carrying the dialect,
/// passed everywhere `ToolSignals::from_exchanges` is called today.
///
/// A rename does *not* cost the bare spelling anything, because the bare
/// spelling never carried a namespace to rename — see [`CONTROL_TOOL_NAMES`].
...
/// stored record but the name. See [`is_control_call`] for what that costs.
...
/// task.
///
/// **Two spellings, because two clients spell one call two ways** (M12, R-M0).
/// The flat spelling — `mcp__roundhouse__status` — is what a client that folds
/// the namespace into the tool name sends, and it is matched on the namespace
/// *and* the delimiter together, never on the namespace alone: a second MCP
/// server called `roundhouse_extra` flattens to `mcp__roundhouse_extra__…`,
/// which a bare `starts_with` would swallow into our own control traffic and
/// quietly exempt somebody else's tools from every signal in the trigger.
///
/// The bare spelling — `status` — is what the *Responses* wire leaves in the
/// log. Codex advertises an MCP server to the model as a single `namespace`
/// object listing each tool under its own bare name, so the call comes back as
/// two wire fields (`{"name":"status","namespace":"mcp__roundhouse"}`) and
/// canonicalization keeps only the first. Until M12 this classifier saw that
/// name and said "agent work", so the exclusion G04 exists for had never once
/// fired on the surface roundhouse actually ships — an agent polling `status`
/// because our own generated skill told it to accrued repeat-detection and
/// no-progress signal against it.
///
/// **What the bare arm costs, and why it is still the right trade.** The
/// namespace is not in the log, so the *deployment's configured namespace
/// cannot participate in recognising a bare name* — there is nothing to
/// compare it against. Recognition is therefore by name alone, and a third
/// party's MCP server offering a tool called `status` is indistinguishable
/// from ours once the wire has dropped the namespace: its calls are exempted
/// too. That mis-exclusion under-counts an agent's work by a call or two.
/// The failure it replaces over-counted roundhouse's own chatter as the
/// agent's work and fired steers at an agent that had done nothing wrong,
/// which is the louder error and the one G04 names. Closing the gap properly
/// means keeping the namespace in the stored record, which moves the canonical
/// form of every already-stored tool-using session — a decision above this
/// function.
pub fn is_control_call(name: &str) -> bool {
    is_flat_control_call(name) || CONTROL_TOOL_NAMES.contains(&name)
}

/// The flat spelling alone: `mcp__roundhouse__<tool>`, any tool.
///
/// Separate from [`is_control_call`] because the two halves answer different
...
fn is_flat_control_call(name: &str) -> bool {
...
mod tests {
    ...
    fn a_flat_control_call_is_recognised_and_dropped_from_the_task_view() {
        let name = format!("{CONTROL_TOOL_NAMESPACE}{CONTROL_TOOL_DELIMITER}status");
        assert!(is_control_call(&name));

        let items = vec![call("c1", &name, "{}"), call("c2", "grep", "{}")];
        let exchanges = exchanges(&items);
        let task = task_exchanges(&exchanges);
        assert_eq!(
            task.iter().map(|e| e.name.as_str()).collect::<Vec<_>>(),
            vec!["grep"],
    ...
    ///
    /// **This assertion is unchanged; only the `#[ignore]` above it is gone.**
    /// It landed red, as M12's Determine stage wrote it, under
    /// `#[ignore = "R-M0: the Responses wire stores an MCP call under its bare
    /// name …"]` — the finding that `is_control_call`, which matched the flat
    /// `mcp__roundhouse__` prefix alone, had never once fired on the surface
    /// this product actually ships on.
    ///
    ...
    /// [`task_exchanges`] as ordinary work, so a session whose agent polls
    ...
    ///
    /// The fix is [`CONTROL_TOOL_NAMES`], not the deployment's configured
    /// namespace: the namespace is not in the record, so there is nothing for a
    /// configured value to be compared against. See [`is_control_call`] for
    /// what recognising a bare name by name alone costs instead.
    #[test]
    ...
    /// Every control tool, in both spellings, and the near misses on each.
    ///
    ...
    fn both_spellings_of_every_control_tool_are_ours_and_the_near_misses_are_not() {
        for tool in CONTROL_TOOL_NAMES {
            assert!(is_control_call(tool), "the bare spelling of `{tool}`");
            assert!(
                is_control_call(&format!(
                    "{CONTROL_TOOL_NAMESPACE}{CONTROL_TOOL_DELIMITER}{tool}"
                )),
        ...
        for theirs in [
            ...
            assert!(!is_control_call(theirs), "`{theirs}` is not ours");
        }
    }
    ...
    /// red and be deleted.
    #[test]
    fn a_third_partys_bare_status_tool_is_exempted_with_ours_and_that_is_the_price() {
        let items = vec![call("c1", "status", "{}"), call("c2", "grep", "{}")];
        let bare = exchanges(&items);
        assert_eq!(
            task_exchanges(&bare)
                .iter()
                .map(|e| e.name.as_str())
        ...
        // classified exactly.
        let flat_items = vec![
            call("c1", "mcp__other__status", "{}"),
            call("c2", "grep", "{}"),
        ];
        let flat = exchanges(&flat_items);
        assert_eq!(
            task_exchanges(&flat)
                .iter()
                .map(|e| e.name.as_str())

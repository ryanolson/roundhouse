...
pub fn convert(body: &str, request: &ImportRequest<'_>) -> Result<Import, ImportError> {
    ...
    // settled rather than an omission. A catalog entry is `deny_unknown_fields`,
    // so an attribution key here would either fail to load or force a schema
    // this project invented for somebody else's data onto every catalog in
    // existence. The attribution rides in the provenance file instead, and the
    // server reads *that* file beside the catalog to cite it on the one surface
    // where a derived number is republished. Adding the field would buy
    // duplication of an obligation that already has a carrier.
    let fragment = json!({
        "$comment": [
            ...
            "The paired provenance file is what makes these numbers republishable: it carries \
             ...
             field here, because a catalog entry is deny_unknown_fields.",
...
mod tests {
    ...
    /// is the one remedy ruled out: a catalog entry is `deny_unknown_fields`,
    /// so such a fragment would either fail to load or force somebody else's
    /// schema onto every catalog in existence. What was actually missing was
    /// the reading end — nothing in a shipped binary opened the provenance

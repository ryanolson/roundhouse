...
pub enum ImportError {
    ...
    #[error(
        "`{model}` has no attribution: `meta.citation` is null (which is what a response \
         ...
         republishing this data, and roundhouse's savings dashboard republishes figures \
         derived from it -- it prices the routing saving through the capability gate these \
         priors feed, and cites the provenance file this entry would have been written into"
    )]
...
pub fn convert(body: &str, request: &ImportRequest<'_>) -> Result<Import, ImportError> {
    ...
    // No `attribution` field on a fragment entry, and that is the decision G12
    // settled rather than an omission. A catalog entry is not `deny_unknown_fields`
    // -- that guard sits on the catalog file itself and does not reach nested
    // structs -- so an attribution key on an entry would just be silently
    // ignored rather than refused; the actual cost of adding it as a real
    // field on `FrontierModelSpec` is forcing a schema this project invented
    // for somebody else's data onto every catalog in existence, since every
    // field there is required. The attribution rides in the provenance file
    // instead, and the server reads *that* file beside the catalog to cite it
    // on the one surface where a derived number is republished. Adding the
    // field would buy duplication of an obligation that already has a
    // carrier.
    let fragment = json!({
        "$comment": [
            ...
            "The paired provenance file is what makes these numbers republishable: it carries \
             ...
             to each raw score. Keep the two together -- there is deliberately no attribution \
             field here: adding one would force every catalog to carry it, since every field \
             on a catalog entry is required.",
            "Put the provenance file beside the file ROUNDHOUSE_CATALOG names, under its \
             default name quality-prior.provenance.json, and the savings dashboard cites it \
             for you under the figure these priors price."
        ],
        "models": entries,
    });
    ...
    let provenance = json!({
        "$comment": [
            "Provenance for the catalog fragment generated beside this file. OpenRouter \
             ...
             savings dashboard republishes figures derived from it, so this file travels \
             with the numbers -- and is read by the server, beside the catalog, to render \
             the citation below that figure."
        ],
...
mod tests {
    ...
    /// G12: the attribution the refusal defends reaches the surface that
    /// republishes the number — through the provenance file, which is its
    /// carrier, and not through the fragment.
    ///
    /// The finding proposed an `attribution` field on each fragment entry. That
    /// is the one remedy ruled out: a catalog entry is not `deny_unknown_fields`
    /// -- that guard sits on the catalog file itself, not its nested entries --
    /// so the field would not be refused; it would force somebody else's
    /// schema onto every catalog in existence instead, since every field on a
    /// catalog entry is required. What was actually missing was the reading
    /// end — nothing in a shipped binary opened the provenance
    /// file — and that is now `catalog_config::quality_prior_citation`, pinned
    /// by `roundhouse-server/tests/review_m10_g12.rs`.
    ///
    /// So this test states the division of labour the two files are under: the
    /// fragment carries identity and the number and *nothing else*, the
    /// provenance carries the attribution for every entry the fragment names,
    /// and neither carries the other's job. A future edit that "helpfully" adds
    /// an attribution key to the fragment breaks the first assertion here
    /// before it breaks a deployment's catalog load.
    #[test]
    fn the_attribution_the_refusal_defends_can_reach_the_surface_that_republishes_it() {
        ...
        for entry in models {
            ...
            assert_eq!(
                keys,
                vec!["model", "provider", "quality_prior"],
                "a fragment entry is identity and the number: an attribution key \
                 here would be refused by the catalog boundary it is meant to be \
                 merged into: {entry}"
            );
        }

        // And every model the fragment names is attributable from the file that
        // travels with it -- which is what the server reads beside the catalog.
        let entries = import.provenance["entries"]
            .as_array()
            .expect("entries is an array");
        ...
        for entry in entries {
            ...
            assert!(
                attributable,
                "every entry the fragment republishes has to be creditable from \
                 the provenance file, or the refusal upstream is defending \
                 nothing: {entry}"
            );
        }
    }

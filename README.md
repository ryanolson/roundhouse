...
> steered by text rather than by a tool call; and `topham`, the operator
> entry point that turns a saved profile into a running agent on either
> client and either topology.
>
...
> client sends on every `tools/call`, and MCP wiring for Claude Code — which
> that client takes as a settings file rather than as environment — is not
...
as `<set>`. It also prints the limits no refusal can close — under a
subscription login an *interactive* Claude Code session asks once before it will
use the API key, and a chained **codex** run is unproven because Relay splices
its own `--config model_provider=…` onto the client's argv and that override
outranks the generated `config.toml`.
...
close. `claude_e2e` gains two tests that drive the real client *through a real
`topham`* — one `topham launch` on Direct, one `topham relay` on Chained — and
assert at roundhouse's edge exactly what the hand-built tests assert. The child
they spawn is handed a turn key, two homes and a `PATH` and **no `ANTHROPIC_*`
variable at all**, so a launcher that resolved the profile wrongly cannot pass
by inheriting anything. `codex_e2e` gains the same shape (no `codex` binary is
available where it was written, so it has never been run):
...
above — but three things around it are stated rather than solved. **MCP wiring
for Claude Code** is not generated by anything: that client takes its MCP
servers as a settings document rather than as environment, so a launched Claude
Code reaches the Messages surface and not the `/mcp` mount. **Chained codex is
unproven**, because Relay splices `--config model_provider=…` onto the client's
argv and a codex `--config` override outranks the generated `config.toml`, so
the turn-key header that config names is not what the client presents;
`topham plan` says so on that profile rather than refusing it, since the remedy
(Relay's own `openai_auth_header`) is the fallback wiring the runbook records as
deliberately untested. And a `RoundhouseKey` profile under an existing
subscription login still asks once in an *interactive* session before the API
key is used — also stated in `topham plan`'s output, not solved.
The MCP surface
